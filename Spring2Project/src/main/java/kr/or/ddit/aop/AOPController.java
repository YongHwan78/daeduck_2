...
public class AOPController {
	 ...
	 * 		신입 채민이는 부탁받은 요구사항을 이행하기 위해서 본인이 만들고 있는 서비스 로직에서 처리 속도를 찎어볼 메소드를
	 * 		개발해 처리 속도가 잘 찍히는걸 확인합니다.
	 * 		너무 기쁜 마음으로 진원팀장님께 단번에 쫒아가 보고합니다.
	 * 		진원팀장님은 아주 긍정적인 검토안을 채민사원한테 전달하면서 우리 서비스 전체에도 각 처리속도를 찍어주세요! 라고
	 * 		다시 부탁합니다. 채민사원은 본인이 만들어 낸 메소드를 각 기능별 서비스 로직에 하나하나씩 약 2만개 쯤 작성 하던중,
	 * 		의문을 갖습니다...
	 * 
	 ...
	 * 
	 * 		하는 생각을 하게 됩니다.
	 * 		여기서, 시간을 측정하고 권한을 체크하는 동의 기능은 옵션과 같은 부가기능으로 일종의 은프라 로직이라고 하는데,
	 ...
	 *      		간단하게 설명하면 관점 지향 프로그래밍은 횡단 관심사의 분리을 실형하는 방법이다.
	 *      
	 *      			- 횡단 관심사(Cross-Cutting-Concern)
	 ...
	 *      		(3) 핵심 비즈니스 로직 대상 객체에 어떤 관심사들을 겹합할 것인지를 설정한다.
	 *      
     ...
     *					: 어디에 적용할 것인지 결정, 메소드/객체생성시/필드점근시 등등
     *			
     *				- 어드바이스(Advice)
     *					: 특정 조인 포인트에서 실행되는 코드로, 횡단 관심사를 실제로 구현해서 처리하는 부분이다.
     *					: 어던 부가 기능을 구현할 것인지 결정
     ...
     *		여기서 등장하는 개념이 프록시인데, 프록시는 쉽게 말하면 대리자라고 생각하면 되는데 타겟으로 향하는 요청을 중간에서
