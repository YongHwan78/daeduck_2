...
public class TransactionController {
	 ...
	 * 			하지 않도록 설계되어 있다.
	 * 			기본적으로 스프링에서 트랜잭션 처리는 RuntimeException계열(Unchecked Exception)이라면 Rollback
	 * 			처리를 한다.
	 * 
	 ...
	 * 			─────────────────────────────────────────────────────────────────────────────
	 * 			예외 처리 여부	| 반드시 예외 처리 코드가 있어야 한다.		|	강제로 예외처리는 아니다.
	 ...
	 * 						하더라도 자신의 트랜잭션에서 처리한 내용만 사용하는것을 의미
	 * 
	 * 				- DEFAULT : 기본 데이터 저장소의 기본 격리 수준을 사용합니다.
	 * 					> 우리가 사용하는 Oralce은 READ_COMMITED를 사용하며, MySQL은 REPEATABLE_READ를
	 * 						기본 격리 수준으로 사용한다.
	 ...
	 * 				- READ_UNCOMMITED : Dirty read, Non-repeatable Read 및 Phantom ream가 발생할 수 
	 * 									있음을 나타내는 상수입니다.
	 ...
	 * 					> 트랜잭션 Commit이 일어난 데이터에 대해서 조회가 가능(트랜잭션 완료 시까지 조회에 대한 
	 * 						Shared lock이 걸리진 않음)
	 ...
	 * 					처리할지에 대한 설정
	 * 				- 추가적인 트랜잭션을 기존 트랜잭션에 포함 시켜 함께 처리할 수도 있고, 추가적은 트랜잭션처럼 별도의
	 ...
	 * 					> 무조건 새로운 트랜잭션을 시작한다.
	 * 					> 기존 작업중인 트랜잭션이 존재한다면, 기존 트랜잭션을 잠시 보류하고 새롭게 만들어진 트랜잭션을
	 * 						이행 후 기존 트랜잭션을 실행한다.
	 * 					> rollback은 각각 이루어진다.
	 * 
	 ...
	 * 					> 클래스라인에 선언적 트랜잭션 어노테이션이 걸려있을때, 특정 메서드만 트랜잭션을 처리하지 않겠다고
	 * 						했을때 사용할 수 있는 속성
	 * 
	 * 			3) readOnly (읽기전용 설정)
