...
public class HomeController {
	...
	// 해당경로 : D:/workspace/.metadata/.plugins/,,,/Spring2Project/
	...
	/*
	 * 		5. 컬렉션 Map 타입
	 * 		- Map 형태의 컬렉션 자료를 JSON 객체 타입의 데이터로 만들어서 반환하는 용도로
	 * 		사용한다.
	 */
	...
	public ResponseEntity<Void> goHome0601(){
		...
		// 응답으로 URL이 응답으로 나간걸 확인할 수 있는데, 이때 상태코드 200으로 정상
		// 응답이 나간걸 확인할 수 있다.
		// 그리고, 다른 기능으로 아무런 형태 없이 응답으로 나가지만 응답의 대한 header
	 ...
	 * 		- response 할 때 HTTP 헤더 정보와 객체 데이터를 Map 형태로 전달하는
	 *		용도로 사용한다.
	 */
	 ...
	 * 		- response 할 때 HTTP 헤더 정보와 바이너리 파일 데이터를 전달하는 
	 * 		용도로 사용한다.
	 * 		- 파일을 처리하기 위해서 의존라이브러리인 commons-io를 추가한다.(pom.xml)
	 * 
	 * 		무료/유료 이미지 다운로드 홈페이지를 사용해보면 이미지 미리보기 또는 미리보기 
	 * 		후 다운로드를 할 수 있는 기능이 제공된다.
	...
	public ResponseEntity<byte[]> goHome1101(){
		...
		try {
			in = new FileInputStream("D:\\00.JSP_SPRING\\02.SPRING2\\Anne-Marie.jpg");
