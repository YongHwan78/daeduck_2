...
public class FileUploadController02 {
	 ...
	 * 			- 파일업로드 목록 화면 페이지 만들기 (item2/list.jsp)
	 * 			- 여기까지 확인
	 * 
	 ...
	 * 			- 파일업로드 수정 화면 페이지 만들기 (item2/modify.jsp)
	 * 			- 여기까지 확인
	 * 
	 * 			- 파일업로드 수정 기능 컨트롤러 메소드 만들기 (item2Modifty:post)
	 ...
	 * 			- 파일업로드 삭제 화면 페이지 만들기 (item2/remove.jsp)
	 * 			- 여기까지 확인
	 * 
	...
	public String item2Register(Item2 item, Model model) throws Exception {
		...
		for(int i = 0; i < pictures.size(); i++) {
			...
			if(i == 0) {	// 첫번째 파일 URL정보 설정
				item.setPictureUrl(savedName);
			}else if(i == 1) { // 두번째 파일 URL정보 설정
	...
	public String item2Modifty(Item2 item, Model model) throws Exception {
		List<MultipartFile> pictures = item.getPictures();
