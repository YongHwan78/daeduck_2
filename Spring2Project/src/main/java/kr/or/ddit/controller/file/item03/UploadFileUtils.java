...
public class UploadFileUtils {
	...
	public static String uploadFile(String uploadPath, String originalName, byte[] fileData) 
		...
		// 배포된 서버 업로드 경로 + /2024/05/29/ + UUID_원본파일명으로 File target을 하나 만들어놓는다.
	...
	private static void makeThumnail(String uploadPath, String savedPath, String savedName) 
			throws Exception {
	...
	private static void makeDir(String uploadPath, String...paths) {
		...
		for(String path : paths) {
			...
			// /2024/05/29 과 같은 경로에 각 폴더가 없으면 각각 만들어진다.
			if(!dirPath.exists()) {
				dirPath.mkdirs();
			}
		}
