...
public class NoticeDownloadController {
	...
	public View noticeDownload(int fileNo, ModelMap model) {
		...
		noticeFileMap.put("fileName", noticeFileVO.getFileName());	// 파일명추가
		noticeFileMap.put("fileSize", noticeFileVO.getFileSize());	// 파일크기 추가
		noticeFileMap.put("fileSavepath", noticeFileVO.getFileSavepath());	// 파일저장경로 추가
		...
		// 그 클래스는 AbstractView를 상속받아 renderMergedOutputModel 함수를 재정의할 때 View로 취급될 수 있게함
