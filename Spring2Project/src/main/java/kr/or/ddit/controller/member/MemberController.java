...
//			LoggerFactory.getLogger(MemberController.class);
	...
	// 4) HTML Form 필드값이 숫자일 경우에는 숫자로 타입변환하여 데이터를 취득할 수 있다.
	@RequestMapping(value="/register02", method = RequestMethod.POST)
	public String register02(
			String userId, String password, int coin
		) {
	...
	@RequestMapping(value="/beans/register01", 
			method = RequestMethod.POST)
	...
	@RequestMapping(value="/beans/register02", 
			method = RequestMethod.POST)
	...
	public String registerCheckbox03(ArrayList<String> hobbyList) {
		...
		// 받는 타입을 List로 하게되면 No primary or default constructor found for
	...
	public String registerUser() {
		...
		// result 페이지에서 넘긴 모든 데이터는 영문으로 된 값 그대로를 출력하는게 아니라
		// 한글로 바꿔서 출력해주세요.
		...
		// member/registerAllResult 페이지로 이동해서 출력하는데 JSTL을 활용하여 EL표현문으로 
		// 데이터를 출력!
	 ...
	 */
	// 3) 파일업로드 폼 파일 요소값과 텍스트 필드 요소값을 MultipartFile 매개변수와 자바빈즈 매개변수로
	// 처리한다.
	...
	// 여러개의 파일업로드를 폼 파일 요소값을 MultipartFile 타입의 요소를 가진 리스트 컬렉션 타입
	// 매개변수로 처리한다.
	// :::: 기본 타입 컬렉션 리스트로는 파일 데이터를 받을 수 없다. XXXXXXXXXX
