...
public class NoticeLoginController {
	...
	public String signup(
		...
		if(errors.size() > 0) {	// 넘겨받은 데이터가 비정상(에러 발생)
			...
			if(result.equals(ServiceResult.OK)) {	// 회원가입 성공
				// 회원가입 완료 후, 일회성 메세지를 처리하기 위해 message 등록
	...
	public ResponseEntity<String> idForgetProcess(@RequestBody NoticeMemberVO memberVO) {
		return new ResponseEntity<String>(noticeService.idForgetProcess(memberVO), HttpStatus.OK);
	}
	
	// 비밀번호 찾기
//	@RequestMapping(value="/pwForget.do", method = )
//	public aaaa pwForgetProcess() {
//		
//	}
